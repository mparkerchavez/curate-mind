...
**Update (July 30, 2026):** The manual move is now the fallback, not the routine. All three intake tools accept an optional `capturedAt` (the capture date in `YYYY-MM-DD`), which overrides "now" for the week folder, the filename date, and the capture metadata line. `cm_extract_pdf` gained it first on July 14; `cm_fetch_url` and `cm_fetch_youtube` follow here, because a link saved on a phone or a video queued for later is captured when the curator saves it, not when the fetch tool happens to run. Behavior is unchanged when the parameter is omitted. The date is parsed as a local calendar day so week boundaries do not shift under a UTC-midnight reading, and impossible dates are rejected rather than rolled over into a real one. Capture date is never inferred from file timestamps: cloud sync and file copies rewrite those, which is part of what caused the original misfiling.
...
**The implementation pattern:** `cm_correct_anchor` handles `anchor_text`, `anchor_passage`, `anchor_missing`, and two-DP `anchor_swap` corrections. `cm_correct_attribution` handles source metadata fixes (`source_publisher`, `source_author`, `source_url`, `source_published_date`, and `source_tier`) and `dp_speaker_attribution`. The write happens in one Convex mutation: insert the `corrections` row, then patch the target field. For anchor swaps, both correction rows and both data point patches happen atomically in the same mutation. `cm_get_source_corrections` reads a source's full correction history, mirroring `cm_get_data_point_corrections` for data points.

**Guardrails:** Anchor corrections require 10-40 words and a reason of at least 10 characters. The tool checks the source `fullText` for a case-insensitive, whitespace-tolerant substring match and returns a warning when no match is found, but allows the curator to proceed. Source URL corrections must parse as HTTP(S) URLs. Published dates must be valid `YYYY-MM-DD` ISO dates. Source tier corrections must be 1, 2, or 3 and must differ from the current tier (no-op corrections are rejected); the tier is stored as its string form in the audit row and patched as the numeric literal on the source. Speaker attribution is structured metadata, not a claim rewrite.
...
**Scope:** data points only. `supersedeSource` keeps its write-once lineage pointers; the `lifecycleEvents` table already carries `targetType: "source"` so extending it later needs no schema change. Do not assume the source rule from the data point rule.
