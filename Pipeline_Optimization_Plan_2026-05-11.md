...
### 2026-05-11 — Post-completion cleanup: loose threads
- **Status:** Completed
- **What changed:**
  - `mcp/src/tools/extraction.ts` — added explicit callback types (`{ dataPointId: string }` on `enrichBatch` result map; `TagBatchResult` local type alias + explicit `sum: number, r: TagBatchResult` on both `reduce` calls and the trailing `map` in `cm_update_data_points_tags_batch`).
  - `mcp/src/tools/synthesis.ts` — added explicit callback types (`position: unknown` on the `ActivePositionForLens` type guard filter; `p: ActivePositionForLens` on three `.map`/`.flatMap` callbacks; `q: string` on the open-questions `.map`).
  - `mcp/src/scripts/sourceLinkBackfill.ts` — added `sourceId: unknown` annotation on the `sourceIds.map` callback.
  - All `src/` TypeScript errors cleared (`npx tsc --noEmit` reports zero `src/` errors).
- **Deviations from plan:** The `convex/*.ts` errors noted in the Prompt 5 log are pre-existing and architectural: `api.d.ts` (generated by Convex CLI) imports the Convex source files, which pulls them into `mcp`'s strict TypeScript compilation transitively. Fixing those requires adding proper Convex-generated type annotations (`QueryCtx`, `MutationCtx`, typed `q` callbacks) to every handler in the Convex backend — a separate task if a fully clean build is ever needed.
- **New follow-ups discovered:** None.
- **Next chat should know:** `cm-deep-extract/SKILL.md` has no reference to `cm-source-pipeline` and the `skills/cm-source-pipeline/` folder does not exist — that thread was already resolved by earlier prompt work. The SIGALRM limitation in `_run_with_alarm` is already documented in the function's docstring.
