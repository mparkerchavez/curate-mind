...
5. Week-folder rule: source folders represent the week the curator **captured** (downloaded or saved) a source, not the week it happened to be processed. All three intake tools (`cm_extract_pdf`, `cm_fetch_url`, `cm_fetch_youtube`) default to filing by the week that is current when they run, and all three accept an optional `capturedAt` (the capture date in `YYYY-MM-DD`) that overrides it. When a source was captured in an earlier week than the one you are processing in, pass `capturedAt` so the markdown, its filename date, and its capture metadata line all land in the capture week's folder. Only if a source was already processed without `capturedAt` do you need the manual repair: move the markdown (and, for a PDF, the original file) back into the capture week's folder after ingestion, and update `review-status.json` in both folders (add the entry to the capture week, remove it from the processing week).
...
- **Source lineage pointers are still set once and cannot be re-pointed (Decision 38).** Data point lifecycle and source lineage now differ on this; do not assume the source rule from the data point one.
- Retiring or replacing a single data point uses `cm_supersede_data_point`; reversing that uses `cm_restore_data_point` (admin toolset, curator-only). Linking a re-ingested source to the one it replaced uses `cm_supersede_source`. Superseded/retired data points are excluded from live evidence (cm_ask, cm_search, public routes, tag retrieval) but stay fetchable by id.
- **Retiring a source does NOT retire its data points.** The two lifecycles are independent and no read path consults the parent source, so a superseded source leaves its evidence live until each data point is retired separately. Check `cm_get_source_usage` before assuming otherwise; it reports live counts, plus `restoreCount` where a decision has been reversed.
